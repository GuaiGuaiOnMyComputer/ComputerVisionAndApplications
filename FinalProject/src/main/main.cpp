...
int main(int, char**)
{
    ...
    std::list<const cv::Point3d*> allValidPredictedWorldPoint_ptrs;
    ...
    for (size_t i = 0; i < scanImageIo.GetImageCount(); i++)
    {
        ...
        std::list<const cv::Point3d *> validWorldPoint_ptrs;
        ...
        finprj::FeatureMatching::filter_nonmatching_points(bluePixelCoors_left, bluePixelCoors_right, validBluePixelCoors_left, validBluePixelCoors_right, validPointCount);
        ...
        pointProjection.FilterOutliners(pointProjection.GetRightP(), currentImagePair.RightRoi.size(), validWorldPoint_ptrs);
        pointProjection.FilterOutOfBoundPoints(allPredictedWorldPoints[i], cv::Range(-200, 200), cv::Range(-200, 200), cv::Range(-300, 300), allValidPredictedWorldPoint_ptrs);
    }
